...
export class CategoriasServiciosService {
  ...
  findAll(vigente?: boolean): Observable<Categoria[]> {
    ...
    return this.http.get<Categoria[]>(`${this.apiUrl}/categorias-servicios`, { params });
  }

  create(data: CreateCategoriaRequest): Observable<Categoria> {
    return this.http.post<Categoria>(`${this.apiUrl}/categorias-servicios`, data);
  }

  update(id: number, data: UpdateCategoriaRequest): Observable<Categoria> {
    return this.http.put<Categoria>(`${this.apiUrl}/categorias-servicios/${id}`, data);
  }

  remove(id: number): Observable<Categoria> {
    return this.http.delete<Categoria>(`${this.apiUrl}/categorias-servicios/${id}`);
  }
}
